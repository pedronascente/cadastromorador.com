...
class FichaCadastral extends MY_Controller {
    public $_dados_session;
    public $_nome_vendedor;
    public $_email_vendedor;
    ...
    public function show() {
        ...
        $this->load->view('fichaCadastral/index', array(
    ...
    public function editar_cadastro_morador(){
        $is_valid_token_chave = $this->ImovelModel->validar_token_chave([
            'token'=>$this->input->get('token'),
            'chave'=>$this->input->get('chave')
        ]);
        if($is_valid_token_chave){
            $_SESSION['CONDOMINIO'] = $this->CondominioModel->findByIdJoin(['token'=>$this->input->get('token')],'id_condominio,nome,token,dataCriacao,imovel_id_imovel,nome_usuario,email');
            $_SESSION['CHAVEIMOVEL'] = $this->input->get('chave'); 
            $CONDOMINIO = $_SESSION['CONDOMINIO'];
            $token = trim($this->input->get('token'));
            $chave = trim($this->input->get('chave'));
            //Recuperar imovel com o id do condmonio ea chave do imovel
            $objimovel = $this->ImovelModel->findByIdJoinTipoImovel([
    ...
    public function finalizarCadastro() {
        ...
        $destino_arquivo = 'assets/pdf/fichacadastral.pdf';
        ...
        //Gerar Arquivo PDF :
        $this->GerarPDF($destino_arquivo);
        //Enviar Email :
        if (!$this->sendEmail(
                [
                    'email_destino' => $_SESSION['DADOS']['imovel']->email_responsavelCadastro, 
                    'email_vendedor' => $this->_email_vendedor, 
                    'anexo' => $destino_arquivo, 
                    'email_nome' => $_SESSION['DADOS']['imovel']->nome_responsavelCadastro, 
                    'vendedor' => strtoupper($this->_nome_vendedor),
                ]
            )
        ) {
            $_SESSION['ERROR']['SENDMAIL'] = 'Erro : Não foi possivel enviar seu Email, contacte o Supórte.';
        }
        redirect("sucesso", 'location', 301);
    }

    public function sucesso() {
        unset($_SESSION);
        $this->load->view('fichaCadastral/sucesso');
    }
    ...
    private function sendEmail($dataEmail) {
        ...
        $this->email->reply_to($dataEmail['email_vendedor']);
     ...
     * @return   .pdf  
     */
    private function GerarPDF($destino_arquivo) {
